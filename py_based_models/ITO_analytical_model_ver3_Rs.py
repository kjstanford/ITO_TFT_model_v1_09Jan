...
def ITO_analytical_model(inputs, params):
    ...
    def IdVg(Vg, Vds):
        ...
        Id_TFT = lambda Vs,Vd: integrate.quad( lambda V: (W/L)*q*mu_band*nfree(Efermi,phi_fn(V)-V),Vs,Vds-Vd )\
                + (W/L)*q*mu_band*kB*T*( nfree(Efermi,phi_fn(Vs)-Vs) - nfree(Efermi,phi_fn(Vds-Vd)-Vds+Vd) )\
                + (W/L)*q*mu_band*(-g2D*(kB*T)**2)*\
                ( li2(-exp(-(En(1)-phi_fn(Vds-Vd)+Vds-Vd-Efermi)/(kB*T)))\
                - li2(-exp(-(En(1)-phi_fn(Vs)+Vs-Efermi)/(kB*T))) )
