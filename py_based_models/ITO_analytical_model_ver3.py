...
def ITO_analytical_model(inputs, params):
    ...
    gfree = lambda E: g2D*((E>=En(1)))

    n2D = lambda delE: g2D*kB*T*log(1+exp(-(delE)/(kB*T)))
    n2D_approx = lambda delE: g2D*kB*T*exp(-(delE)/(kB*T))
    nfree = lambda Ef,phi: n2D(En(1)-phi-Ef) #phi is neg of energy
    nfree_approx = lambda Ef,phi: n2D_approx(En(1)-phi-Ef) #phi is neg of energy
    ...
    def IdVg(Vg, Vds):
        ...
        Id_return += (W/L)*q*mu_band*kB*T*(nfree(Efermi,phi_fn(0)) - nfree(Efermi,phi_fn(Vds)-Vds))
        Id_return += (W/L)*q*mu_band*(-g2D*(kB*T)**2)*\
            ( li2(-exp(-(En(1)-phi_fn(Vds)+Vds-Efermi)/(kB*T))) - li2(-exp(-(En(1)-phi_fn(0)-Efermi)/(kB*T))) )
